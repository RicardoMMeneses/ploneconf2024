...
STACK_NAME=ploneconf2024-ricardommeneses-tangrama-com-br
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t ricardommeneses/ploneconf2024-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t ricardommeneses/ploneconf2024-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name ploneconf2024-frontend-acceptance --link ploneconf2024-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ricardommeneses/ploneconf2024-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name ploneconf2024-backend-acceptance -d ricardommeneses/ploneconf2024-backend:acceptance
